...
if($step == 1)
{
    ...
    if($dopost=='regbase')
    {
        ...
        */
        ...
        //检测手机是否存在
        $trow = $dsql->GetOne("SELECT mid FROM `#@__member` WHERE mobile LIKE '$mob' ");
        if(is_array($trow))
        {
        	ShowMsg("手机号{$mob} 已存在，请使用别的手机号！", "-1");
        	exit();
        }
        if($safequestion==0)
        {
        ...
        //二次开发，spaceSta 注册的时候默认为0， 结果是已通过审核
       ...
       '0','$spaceSta','','$safequestion','$safeanswer','$jointime','$joinip','$logintime','$loginip','$mob','$school','$zhuanye','$nianji','$qq','$city','$introduction'); ";
        ...
        if($dsql->ExecuteNoneQuery($inQuery))
        {
            ...
            if($cfg_mb_reginfo == 'Y' && $spaceSta >=0)
            {
                ...
                ShowMsg("注册成功，正在跳转到个人中心","$cfg_cmspath/member/",0,1000);
                exit;
            } 
        } else {
        	echo $dsql->GetError();
