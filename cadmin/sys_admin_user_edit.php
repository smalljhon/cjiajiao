...
if($dopost=='saveedit')
{
	...
	if(empty($cityNames))
    {
        ...
        $cityNames = join(',', $cityNames);
        //if($typeid=='0') $typeid = '';
    }
    if($id!=1){
        $query = "UPDATE `#@__admin` SET uname='$uname',usertype='$usertype',tname='$tname',email='$email',admin_city='$cityNames',typeid='$typeid' $pwd WHERE id='$id'";
    }else{
